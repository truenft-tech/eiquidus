...
module.exports = {
  ...
  tnft_get: function(namespace,key, cb) {
    var cmd = prepareRpcCommand(settings.api_cmds.tnft_get, [namespace,key]);
    ...
    if (!(cmd.method == '' && cmd.parameters.length == 0)) {
      if (settings.api_cmds.use_rpc) {
        ...
        var uri = base_url + 'tnft_get?namespace=' + namespace + ',key=' + key;
