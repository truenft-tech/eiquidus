  ...
  function express_app() {
    ...
    function specialApiCase(method_name, query_parameters) {
      ...
      switch (method_name) {
        case 'tnft_get':
          for (var parameter in query_parameters) {
            if (query_parameters.hasOwnProperty(parameter) && parameter == 'namespace_id')
              params.push(query_parameters[parameter]);
            else if (query_parameters.hasOwnProperty(parameter) && parameter == 'key')
                params.push(query_parameters[parameter]);
          }

          var cmd = prepareRpcCommand(settings.api_cmds.tnft_get, params);
